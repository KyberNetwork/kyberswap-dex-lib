...
const (
	poolTestdataFileName = "pool_simulator_msgpack_testdata.go"
...
func main() {
	...
	outFile, err := os.Create("./msgpack_test.go")
	if err != nil {
	...
	for _, pkgName := range pkgNames {
		...
		fmt.Fprintf(outFileBuf, "func Test_%s(t *testing.T) {\n", strings.TrimPrefix(pkgName, "pkg_"))
		fmt.Fprintf(outFileBuf, "\tutils.TestMarshalUnmarshal(t, %s.MsgpackTestPools())\n", pkgName)
		fmt.Fprintf(outFileBuf, "}\n")
	}
}

func emitImports(outFileBuf io.Writer, pkgNames, importPaths []string) {
	fmt.Fprintf(outFileBuf, "package msgpacktest\n")
	fmt.Fprintf(outFileBuf, "\n")

	fmt.Fprintf(outFileBuf, "// Code generated by %s/pkg/msgpack_test/generate DO NOT EDIT.\n", moduleName)
	fmt.Fprintf(outFileBuf, "\n")

	fmt.Fprintf(outFileBuf, "import (\n")
	fmt.Fprintf(outFileBuf, "\t\"testing\"\n")
	fmt.Fprintf(outFileBuf, "\n")
	fmt.Fprintf(outFileBuf, "\t\"github.com/KyberNetwork/kyberswap-dex-lib/pkg/msgpack/msgpack_test/utils\"\n")
	fmt.Fprintf(outFileBuf, "\n")
	fmt.Fprintf(outFileBuf, "\n")
...
func findAllPoolTestdataSourceFile(rootDir string) []string {
	...
	err := filepath.Walk(rootDir, func(path string, info fs.FileInfo, err error) error {
		...
		if strings.HasSuffix(path, "/"+poolTestdataFileName) {
			paths = append(paths, path)
		}
		return nil
	})
...
func getPackageNamesFromSourceFiles(sourcePaths []string) []string {
	...
	for _, path := range sourcePaths {
		dexName := strings.TrimSuffix(path, "/"+poolTestdataFileName)
...
func getPackageImportPathsFromSourceFiles(sourcePaths []string) []string {
	...
	for _, path := range sourcePaths {
		importPath := filepath.Join(moduleName, strings.TrimSuffix(path, "/"+poolTestdataFileName))
