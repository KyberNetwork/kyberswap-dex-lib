...
func init() {
	if unsafe.Sizeof(poolExporter{}) != unsafe.Sizeof(pancakev3entities.Pool{}) {
	...
	if unsafe.Sizeof(tickListDataProviderExporter{}) != unsafe.Sizeof(pancakev3entities.TickListDataProvider{}) {
...
type poolExporter struct {
	...
	Liquidity        *big.Int
	TickCurrent      int
	TickDataProvider pancakev3entities.TickDataProvider

	token0Price *entities.Price
	token1Price *entities.Price
}

func exportPool(pool *pancakev3entities.Pool) *poolExporter {
...
// tickListDataProviderExporter has the same structure as uniswapv3entities.TickListDataProvider
type tickListDataProviderExporter struct {
	ticks []pancakev3entities.Tick
}

func exportTickListDataProvider(t *pancakev3entities.TickListDataProvider) *tickListDataProviderExporter {
