...
type (
	PoolSimulatorBigInt struct {
		...
		Factory *FactoryBigInt

		gas Gas
	}
)

func NewPoolSimulatorBigInt(entityPool entity.Pool) (*PoolSimulatorBigInt, error) {
	var extra ExtraBigInt
	if err := json.Unmarshal([]byte(entityPool.Extra), &extra); err != nil {
		return nil, err
	}

	var staticExtra StaticExtraBigInt
...
func (p *PoolSimulatorBigInt) _swap0To1(
...
func (p *PoolSimulatorBigInt) _swap1To0(
	tokenAmountIn pool.TokenAmount,
	tokenOut string,
) (*pool.CalcAmountOutResult, error) {
	amountOut := p.getAmountOut(tokenAmountIn.Amount, tokenAmountIn.Token)
	...
	if p.StableSwap && p.Factory != nil && !eth.IsZeroAddress(p.Factory.FeeTo) {
		ownerFeeShare := new(big.Int).Mul(p.FeeDenominator, p.Factory.OwnerFeeShare)
		ownerFee := new(big.Int).Div(
...
func (p *PoolSimulatorBigInt) getAmountOut(amountIn *big.Int, tokenIn string) *big.Int {
...
func (p *PoolSimulatorBigInt) _getAmountOut(
	amountIn *big.Int,
	tokenIn string,
	_reserve0 *big.Int,
	_reserve1 *big.Int,
...
func (p *PoolSimulatorBigInt) _k(balance0 *big.Int, balance1 *big.Int) *big.Int {
	if p.StableSwap {
...
func (p *PoolSimulatorBigInt) _getY(x0 *big.Int, xy *big.Int, y *big.Int) *big.Int {
	for i := 0; i < 255; i++ {
		yPrev := y
		k := _f(x0, y)
