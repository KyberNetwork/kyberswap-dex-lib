...
func (r *VaultReader) readData(ctx context.Context, address string, vault *Vault) error {
	...
	rpcRequest.AddCall(callParamsFactory(vaultMethodUSDR, nil), []interface{}{&vault.USDRAddress})
...
func (r *VaultReader) readTokensData(
	...
	usdrAmounts := make([]*big.Int, tokensLen)
	maxUSDRAmounts := make([]*big.Int, tokensLen)
	...
	for i, token := range vault.WhitelistedTokens {
		...
		rpcRequest.AddCall(callParamsFactory(vaultMethodUSDRAmounts, []interface{}{tokenAddress}), []interface{}{&usdrAmounts[i]})
		rpcRequest.AddCall(callParamsFactory(vaultMethodMaxUSDRAmounts, []interface{}{tokenAddress}), []interface{}{&maxUSDRAmounts[i]})
	...
	for i, token := range vault.WhitelistedTokens {
		...
		vault.USDRAmounts[token] = usdrAmounts[i]
		vault.MaxUSDRAmounts[token] = maxUSDRAmounts[i]
