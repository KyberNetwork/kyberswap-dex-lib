...
func NewVaultScanner(
	...
	return &VaultScanner{
		...
		usdrReader:            NewUSDRReader(ethrpcClient),
...
func (vs *VaultScanner) getVault(ctx context.Context, address string) (*Vault, error) {
	...
	usdr, err := vs.usdrReader.Read(ctx, vault.USDRAddress.String())
	if err != nil {
		vs.log.Errorf("error when usdrReader read: %s", err)
		return nil, err
	}
