...
// UpdateBalance update UsdrAmount only
...
func (p *PoolSimulator) UpdateBalance(params pool.UpdateBalanceParams) {
	...
	usdrAmount := new(big.Int).Div(new(big.Int).Mul(input.Amount, priceIn), PricePrecision)
	usdrAmount = p.vault.AdjustForDecimals(usdrAmount, input.Token, p.vault.USDR.Address)

	p.vault.IncreaseUSDRAmount(input.Token, usdrAmount)
	p.vault.DecreaseUSDRAmount(output.Token, usdrAmount)
...
func (p *PoolSimulator) getAmountOut(tokenIn string, tokenOut string, amountIn *big.Int) (*big.Int, *big.Int, error) {
	...
	usdrAmount := new(big.Int).Div(new(big.Int).Mul(amountIn, priceIn), PricePrecision)
	usdrAmount = p.vault.AdjustForDecimals(usdrAmount, tokenIn, p.vault.USDR.Address)

	// in smart contract, this validation is implemented inside _increaseUsdrAmount method
	if err = p.validateMaxUsdrExceed(tokenIn, usdrAmount); err != nil {
		return nil, nil, err
	}
	...
	feeBasisPoints := p.vaultUtils.GetSwapFeeBasisPoints(tokenIn, tokenOut, usdrAmount)
...
func (p *PoolSimulator) validateMaxUsdrExceed(token string, amount *big.Int) error {
	currentUsdrAmount := p.vault.USDRAmounts[token]
	newUsdrAmount := new(big.Int).Add(currentUsdrAmount, amount)

	maxUsdrAmount := p.vault.MaxUSDRAmounts[token]

	if maxUsdrAmount.Cmp(bignumber.ZeroBI) == 0 {
		return nil
	}

	if newUsdrAmount.Cmp(maxUsdrAmount) < 0 {
		return nil
	}

	return ErrVaultMaxUsdrExceeded
}
