...
func (u *VaultUtils) GetSwapFeeBasisPoints(tokenIn string, tokenOut string, usdrAmount *big.Int) *big.Int {
	...
	feeBasisPoints0 := u.GetFeeBasisPoints(tokenIn, usdrAmount, baseBps, taxBps, true)
	feeBasisPoints1 := u.GetFeeBasisPoints(tokenOut, usdrAmount, baseBps, taxBps, false)
...
func (u *VaultUtils) GetFeeBasisPoints(token string, usdrDelta *big.Int, feeBasisPoints *big.Int, taxBasisPoints *big.Int, increment bool) *big.Int {
	if !u.vault.HasDynamicFees {
		return feeBasisPoints
	}

	initialAmount := u.vault.USDRAmounts[token]
	nextAmount := new(big.Int).Add(initialAmount, usdrDelta)

	if !increment {
		if usdrDelta.Cmp(initialAmount) > 0 {
			nextAmount = constant.ZeroBI
		} else {
			nextAmount = new(big.Int).Sub(initialAmount, usdrDelta)
		}
	}

	targetAmount := u.vault.GetTargetUSDRAmount(token)
