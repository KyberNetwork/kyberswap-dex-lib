...
func _getAmountIn(
	...
	amountIn := new(big.Int).Add(
		new(big.Int).Div(
			new(big.Int).Mul(
				syncswapstable.MaxFee,
				new(big.Int).Sub(x, adjustedReserveIn),
			),
			new(big.Int).Sub(syncswapstable.MaxFee, swapFee),
		),
		integer.One(),
	)
...
func calAmountAfterFee(amountIn, swapFee *big.Int) (*big.Int, *big.Int) {
	...
	var feeIn = new(big.Int).Div(new(big.Int).Mul(amountIn, swapFee), syncswapstable.MaxFee)
	var feeDeductedAmountIn = new(big.Int).Sub(amountIn, feeIn)

	return feeDeductedAmountIn, feeIn
}

func computeDFromAdjustedBalances(xp0, xp1 *big.Int, A *big.Int) *big.Int {
	...
	for i := 0; i < 256; i++ {
		...
		d = new(big.Int).Div(
			...
			new(big.Int).Add(
				new(big.Int).Mul(new(big.Int).Sub(A, bignumber.One), d),
