...
func NewPoolSimulatorBigInt(entityPool entity.Pool, defaultGas int64) (*PoolSimulatorBigInt, error) {
	var extra Extra
...
func (p *PoolSimulatorBigInt) UpdateBalance(params pool.UpdateBalanceParams) {
	si, ok := params.SwapInfo.(StateUpdateBigInt)
	if !ok {
		logger.Warnf("failed to UpdateBalance for Algebra %v %v pool, wrong swapInfo type", p.Info.Address,
			p.Info.Exchange)
		return
	}
