...
type SwapCalculationCacheBigInt struct {
...
type PriceMovementCacheBigInt struct {
...
func (p *PoolSimulatorBigInt) _calculateSwapAndLock(
	zeroToOne bool,
	amountRequired *big.Int,
	limitSqrtPrice *big.Int,
) (error, *big.Int, *big.Int, *StateUpdateBigInt) {
	var cache SwapCalculationCacheBigInt
	var err error

	nextState := &StateUpdateBigInt{}
	...
	var step PriceMovementCacheBigInt
	...
	for i := 0; i < maxSwapLoop; i++ {
		...
		if step.nextTickPrice, err = utils.GetSqrtRatioAtTick(step.nextTick); err != nil {
			return err, nil, nil, nil
		}
		...
		if currentPrice, step.input, step.output, step.feeAmount, err = utils.ComputeSwapStep(
			currentPrice, targetPrice, currentLiquidity, amountRequired, constants.FeeAmount(cache.fee),
		); err != nil {
			return err, nil, nil, nil
		}
		...
		if cache.communityFee.Cmp(integer.Zero()) > 0 {
			delta := new(big.Int).Div(
				...
				COMMUNITY_FEE_DENOMINATOR_BIGINT,
			)
