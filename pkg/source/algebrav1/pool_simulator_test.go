...
func TestComparePoolSimulatorV2(t *testing.T) {
	...
	poolSim, err := NewPoolSimulatorBigInt(*poolEntity, 0)
	require.NoError(t, err)

	poolSimV2, err := NewPoolSimulator(*poolEntity, 0)
	require.NoError(t, err)
	...
	for i := 0; i < 500; i++ {
		...
		t.Run(fmt.Sprintf("test %s WETH -> USDC %d", amt, i), func(t *testing.T) {
			...
			if err == nil {
				...
				assert.Equal(t, remaining(result.RemainingTokenAmountIn), remaining(resultV2.RemainingTokenAmountIn))
...
func TestCloneState(t *testing.T) {
	...
	poolSim, err = NewPoolSimulator(*poolEntity, 0)
	require.NoError(t, err)
