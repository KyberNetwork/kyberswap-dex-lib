...
func (d *PoolTracker) getPoolTicksFromSC(ctx context.Context, pool entity.Pool) ([]Tick, error) {
	...
	for _, chunk := range chunkedWordIndexes {
		...
		resTicks := make([]Tick, 0, len(resp.Request.Calls))
