...
func TestQueriesUniswapV3_GetPoolsListQuery(t *testing.T) {
	...
	t.Run("it should return correct query when allowing subgraph error", func(t *testing.T) {
		...
		actual := getPoolsListQuery("", true, true, big.NewInt(0), 1000, 0)
	...
	t.Run("it should return correct query when subgraph error is not allowed", func(t *testing.T) {
		...
		actual := getPoolsListQuery("", false, false, big.NewInt(0), 1000, 0)

		assert.Equal(t, expect, actual)
	})

	t.Run("it should use the overridden subgraph pool field", func(t *testing.T) {
		expect := fmt.Sprintf(`{
		clPools(
		...
		actual := getPoolsListQuery("clPools", true, false, big.NewInt(0), 1000, 0)
