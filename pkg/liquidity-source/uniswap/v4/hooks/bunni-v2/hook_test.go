...
func Test_Pool_USDC_USDT(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, "999359163", got.TokenAmountOut.Amount.String())
	...
	assert.NoError(t, err)
	assert.Equal(t, "1000615191", got.TokenAmountOut.Amount.String())
}
...
func Test_Quoter(t *testing.T) {
	...
	_, err := rpcClient.R().SetBlockNumber(big.NewInt(23166617)).AddCall(&ethrpc.Call{
		ABI:    bunniQuoterABI,
