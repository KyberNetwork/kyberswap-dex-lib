	...
	hookExtra.Vaults = [2]Vault{
		{
			...
			DepositRate:  uint256.MustFromBig(depositRates[0]),
			MaxDeposit:   uint256.MustFromBig(maxDeposits[0]),
			WithdrawRate: uint256.MustFromBig(withdrawRates[0]),
			MaxWithdraw:  uint256.MustFromBig(maxWithdraws[0]),
		},
		{
			Address:      poolState.Data.Vault1,
			Decimals:     poolState.Data.Vault1Decimals,
			RedeemRate:   uint256.MustFromBig(redeemRates[1]),
			DepositRate:  uint256.MustFromBig(depositRates[1]),
			MaxDeposit:   uint256.MustFromBig(maxDeposits[1]),
			WithdrawRate: uint256.MustFromBig(withdrawRates[1]),
			MaxWithdraw:  uint256.MustFromBig(maxWithdraws[1]),
		},
	}
