...
func (p *PoolSimulator) UpdateBalance(params pool.UpdateBalanceParams) {
	...
	if params.TokenAmountIn.Token == p.Token0.Address {
		...
		p.ZeroToOnePriceLevels = getNewPriceLevelsState(amtInAfterDecimals, p.ZeroToOnePriceLevels)
	} else {
		amtInAfterDecimals := amtInF / math.Pow10(int(p.Token1.Decimals))
		p.OneToZeroPriceLevels = getNewPriceLevelsState(amtInAfterDecimals, p.OneToZeroPriceLevels)
	}
