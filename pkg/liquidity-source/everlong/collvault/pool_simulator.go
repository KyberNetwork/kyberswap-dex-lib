...
// (CollateralRebalancerSwapper): a two-token venue between the stable (KAI) and the
// volatile (vbWBTC) legs whose price comes from the deployed CollateralRebalancer's CR
...
func (s *PoolSimulator) CalcAmountOut(params pool.CalcAmountOutParams) (*pool.CalcAmountOutResult, error) {
	...
	state := &s.Extra
	if cp.stateRegion(state.Collateral, state.Debt, state.PriceWad) != regionStrict {
		// recovery (past the CR wall) / degenerate states are intentionally not ported —
		// refuse to quote rather than risk a wrong price.
		return nil, ErrNotPriceable
	}
...
func (s *PoolSimulator) calcLeverage(params pool.CalcAmountOutParams,
	...
	_, newColl, newDebt := cp.leverageQuote(state.Collateral, state.Debt, state.PriceWad,
		cp.LeverageRatioWad, state.SpreadPpm, shares)
...
func (s *PoolSimulator) calcDeleverage(params pool.CalcAmountOutParams,
	...
	sharesOut, newColl, newDebt, recovery := cp.deleverageQuote(state.Collateral, state.Debt,
		state.PriceWad, cp.LeverageRatioWad, state.SpreadPpm, gross)
	if recovery {
		return nil, ErrNotPriceable
	}
	if sharesOut.Sign() == 0 {
