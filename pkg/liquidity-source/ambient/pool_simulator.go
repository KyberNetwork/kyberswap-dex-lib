...
func (s *PoolSimulator) swap(
	...
	// swapProxy.
	// WONTDO: require(hotPathOpen_); (Skipping)
...
func (s *PoolSimulator) swapExecute(
	base string, quote string, poolIdx *big.Int, isBuy bool, inBaseQty bool, qty *big.Int, tip uint16,
	limitPrice *big.Int, minOutput *big.Int, _ /*reserveFlags*/ uint8,
) (baseFlow, quoteFlow *big.Int, err error) {
	// TODO: preparePoolCntx: query the pool, add poolTip, verify permit swap
	pool, err := preparePoolCntx(base, quote, poolIdx, tip, isBuy, inBaseQty, qty)
	if err != nil {
		return nil, nil, err
	}

	// Chaining.PairFlow memory flow = swapDir(pool, isBuy, inBaseQty, qty, limitPrice);
	flow, err := s.swapDir(pool, isBuy, inBaseQty, qty, limitPrice)
	if err != nil {
		return nil, nil, err
	}

	// (baseFlow, quoteFlow) = (flow.baseFlow_, flow.quoteFlow_);
	baseFlow, quoteFlow = flow.baseFlow, flow.quoteFlow

	// pivotOutFlow(flow, minOutput, isBuy, inBaseQty);
	if _, err := pivotOutFlow(&flow, minOutput, isBuy, inBaseQty); err != nil {
		return nil, nil, err
	}

	// WONTDO: settleFlows(base, quote, flow.baseFlow_, flow.quoteFlow_, reserveFlags);

	// WONTDO: accumProtocolFees(flow, base, quote);

	return baseFlow, quoteFlow, nil
}
...
func (s *PoolSimulator) swapOverPool(dir *swapDirective, p *swapPool) (pairFlow, error) {
	// TODO: snapCurve: get curve from the pool hash
	...
	flow := pairFlow{}
	tickAtSqrtRatio, err := getTickAtSqrtRatio(s.curve.priceRoot)
	if err != nil {
		return pairFlow{}, err
	}
	s.sweepSwapLiq(&flow, s.curve, tickAtSqrtRatio, dir, p)

	// TODO: commitCurve(pool.hash_, curve);
...
func (s *PoolSimulator) sweepSwapLiq(
	...
	if swap.isBuy {
		if !(curve.priceRoot.Cmp(swap.limitPrice) < 1) {
...
*     neighborhood.  */
func (s *PoolSimulator) pinBitmap(poolIdx string, isUpper bool, startTick Int24) (Int24, bool, error) {
...
*         completes. */
func knockInTick(accum *pairFlow, bumpTick Int24, curve *curveState, swap *swapDirective, poolHash string) (Int24, error) {
...
*         AMM liquidity. */
func bumpLiquidity(curve *curveState, bumpTick Int24, isBuy bool, poolHash string) {
...
func pivotOutFlow(flow *pairFlow, minOutput *big.Int, isBuy bool, inBaseQty bool) (*big.Int, error) {
	// outFlow = inBaseQty ? flow.quoteFlow_ : flow.baseFlow_;
	outFlow := new(big.Int)
	if inBaseQty {
		outFlow.Set(flow.quoteFlow)
	} else {
		outFlow.Set(flow.baseFlow)
	}

	// bool isOutPaid = (isBuy == inBaseQty);
	isOutPaid := isBuy == inBaseQty

	thresh := new(big.Int)
	if isOutPaid {
		thresh.Set(minOutput)
		thresh.Neg(thresh)
	} else {
		thresh.Set(minOutput)
	}

	if outFlow.Cmp(thresh) == 1 && minOutput.Cmp(big0) != 0 {
		return nil, errors.New("SL")
	}
...
func preparePoolCntx(base string, quote string, poolIdx *big.Int,
	_ /*poolTip*/ uint16, _ /*isBuy*/ bool, _ /*inBaseQty*/ bool, _ /*qty*/ *big.Int,
) (*swapPool, error) {
	// PoolSpecs.PoolCursor memory pool = queryPool(base, quote, poolIdx);
	p, err := queryPool(base, quote, poolIdx)
	if err != nil {
		return nil, err
	}
