...
func sqrtInt(x *uint256.Int) (*uint256.Int, error) {
	...
	var z = new(uint256.Int).Div(new(uint256.Int).Add(x, big256.BONE), big256.U2)
	var y = x
	for i := 0; i < 256; i += 1 {
		if z.Cmp(y) == 0 {
			return y, nil
		...
		y = z
		z = new(uint256.Int).Div(new(uint256.Int).Add(new(uint256.Int).Div(new(uint256.Int).Mul(x, big256.BONE), z), z), big256.U2)
	}
	return nil, errors.New("sqrt_int did not converge")
}

func halfpow(power *uint256.Int, precision *uint256.Int) (*uint256.Int, error) {
	var intpow = new(uint256.Int).Div(power, big256.BONE)
	var otherpow = new(uint256.Int).Sub(power, new(uint256.Int).Mul(intpow, big256.BONE))
	if intpow.Cmp(uint256.NewInt(59)) > 0 {
		return big256.U0, nil
	}
	var result = new(uint256.Int).Div(big256.BONE, new(uint256.Int).Exp(big256.U2, intpow))
	if otherpow.Cmp(big256.U0) == 0 {
		return result, nil
	}
	var term = big256.BONE
	var x = new(uint256.Int).Mul(big256.U5, big256.TenPow(17))
	var S = big256.BONE
	var neg = false
	for i := 1; i < 256; i += 1 {
		var K = new(uint256.Int).Mul(uint256.NewInt(uint64(i)), big256.BONE)
		var c = new(uint256.Int).Sub(K, big256.BONE)
		if otherpow.Cmp(c) > 0 {
		...
		term = new(uint256.Int).Div(new(uint256.Int).Mul(term, new(uint256.Int).Div(new(uint256.Int).Mul(c, x), big256.BONE)), K)
		if neg {
		...
		if term.Cmp(precision) < 0 {
			return new(uint256.Int).Div(new(uint256.Int).Mul(result, S), big256.BONE), nil
		}
	}
...
func newtonD(ANN *uint256.Int, gamma *uint256.Int, xUnsorted []*uint256.Int) (*uint256.Int, error) {
	...
	for i := 1; i < nCoins; i += 1 {
		var frac = new(uint256.Int).Div(new(uint256.Int).Mul(x[i], big256.BONE), x[0])
	...
	for i := 0; i < 255; i += 1 {
		...
		var _g1k0 = new(uint256.Int).Add(gamma, big256.BONE)
		if _g1k0.Cmp(K0) > 0 {
			_g1k0 = new(uint256.Int).Add(new(uint256.Int).Sub(_g1k0, K0), big256.U1)
		} else {
			_g1k0 = new(uint256.Int).Add(new(uint256.Int).Sub(K0, _g1k0), big256.U1)
		}
		...
		if DPlus.Cmp(DMinus) > 0 {
			...
			D = new(uint256.Int).Div(new(uint256.Int).Sub(DMinus, DPlus), big256.U2)
		}
		...
		var temp = big256.TenPow(16)
		if D.Cmp(temp) > 0 {
			temp = D
		}
		if new(uint256.Int).Mul(diff, big256.TenPow(14)).Cmp(temp) < 0 {
			for _, _x := range x {
				var frac = new(uint256.Int).Div(new(uint256.Int).Mul(_x, big256.BONE), D)
...
func newtonY(ann *uint256.Int, gamma *uint256.Int, x []*uint256.Int, D *uint256.Int, i int) (*uint256.Int, error) {
	...
	for j := 0; j < 255; j += 1 {
		...
		var yfprime = new(uint256.Int).Add(
			new(uint256.Int).Add(new(uint256.Int).Mul(big256.BONE, y), new(uint256.Int).Mul(S, mul2)), mul1,
		)
		var _dyfprime = new(uint256.Int).Mul(D, mul2)
		if yfprime.Cmp(_dyfprime) < 0 {
			y = new(uint256.Int).Div(yPrev, big256.U2)
			continue
		} else {
		...
		var yPlus = new(uint256.Int).Add(
			new(uint256.Int).Div(
				new(uint256.Int).Add(yfprime, new(uint256.Int).Mul(big256.BONE, D)),
				fprime,
			),
			new(uint256.Int).Div(new(uint256.Int).Mul(yMinus, big256.BONE), K0),
		)
		yMinus = new(uint256.Int).Add(yMinus, new(uint256.Int).Div(new(uint256.Int).Mul(big256.BONE, S), fprime))
		...
		if diff.Cmp(t) < 0 {
			var frac = new(uint256.Int).Div(new(uint256.Int).Mul(y, big256.BONE), D)
			if frac.Cmp(big256.TenPow(16)) < 0 || frac.Cmp(big256.TenPow(20)) > 0 {
...
func (t *PoolSimulator) GetDy(i int, j int, dx *uint256.Int) (*uint256.Int, *uint256.Int, error) {
	...
	if xp[j].Cmp(new(uint256.Int).Add(y, big256.U1)) < 0 {
		return nil, nil, ErrReserveViolation
	}

	var dy = new(uint256.Int).Sub(new(uint256.Int).Sub(xp[j], y), big256.U1) // dy: uint256 = xp[j] - y - 1
	xp[j] = y
	if j > 0 {
...
func (t *PoolSimulator) Exchange(i int, j int, dx *uint256.Int) (*uint256.Int, error) {
	...
	dy = new(uint256.Int).Sub(dy, big256.U1)
	if j > 0 {
	...
	if dx.Cmp(big256.TenPow(5)) > 0 && dy.Cmp(big256.TenPow(5)) > 0 {
		...
		if i != 0 && j != 0 {
			...
			p = new(uint256.Int).Div(new(uint256.Int).Mul(_dx, big256.BONE), _dy)
		} else {
			p = new(uint256.Int).Div(new(uint256.Int).Mul(_dy, big256.BONE), _dx)
			ix = i
		}
	}
...
func (t *PoolSimulator) tweakPrice(AGamma []*uint256.Int, _xp []*uint256.Int, i int, pI *uint256.Int, newD *uint256.Int) error {
	...
	if lastPricesTimestamp < blockTimestamp {
		...
		var alpha, _ = halfpow(
			new(uint256.Int).Div(
				new(uint256.Int).Mul(uint256.NewInt(uint64(blockTimestamp-lastPricesTimestamp)), big256.BONE), maHalfTime,
			),
			big256.TenPow(10),
		)
		packedPrices = big256.U0
		for k := 0; k < nCoins-1; k += 1 {
			priceOracle[k] = new(uint256.Int).Div(
				new(uint256.Int).Add(
					new(uint256.Int).Mul(lastPrices[k], new(uint256.Int).Sub(big256.BONE, alpha)),
	...
	if pI.Cmp(big256.U0) > 0 {
		if i > 0 {
			...
			for k := 0; k < nCoins-1; k += 1 {
				lastPrices[k] = new(uint256.Int).Div(new(uint256.Int).Mul(lastPrices[k], big256.BONE), pI)
			}
		}
	} else {
		...
		var dxPrice = new(uint256.Int).Div(__xp[0], big256.TenPow(6))
	...
	for k := 0; k < nCoins-1; k += 1 {
		xp[k+1] = new(uint256.Int).Div(
			new(uint256.Int).Mul(DUnadjusted, big256.BONE), new(uint256.Int).Mul(nCoinsBi, priceScale[k]),
		)
	}
	var xcpProfit = big256.BONE
	var virtualPrice = big256.BONE
	if oldVirtualPrice.Cmp(big256.U0) > 0 {
		var xcp, err = geometricMean(xp, true)
		if err != nil {
			return err
		}
		virtualPrice = new(uint256.Int).Div(new(uint256.Int).Mul(big256.BONE, xcp), totalSupply)
	...
	if new(uint256.Int).Sub(new(uint256.Int).Mul(virtualPrice, big256.U2), big256.BONE).Cmp(
		new(uint256.Int).Add(xcpProfit, new(uint256.Int).Mul(big256.U2, t.AllowedExtraProfit)),
	) > 0 {
	...
	if needsAdjustment {
		...
		var norm = big256.U0
		for k := 0; k < nCoins-1; k += 1 {
			var ratio = new(uint256.Int).Div(new(uint256.Int).Mul(priceOracle[k], big256.BONE), priceScale[k])
			if ratio.Cmp(big256.BONE) > 0 {
				ratio = new(uint256.Int).Sub(ratio, big256.BONE)
			} else {
				ratio = new(uint256.Int).Sub(big256.BONE, ratio)
			}
		...
		if norm.Cmp(
		...
		) > 0 && oldVirtualPrice.Cmp(big256.U0) > 0 {
			var temp, err = sqrtInt(new(uint256.Int).Div(norm, big256.BONE))
			if err != nil {
				return err
			}
			norm = temp
			...
			for k := 0; k < nCoins-1; k += 1 {
				xp[k+1] = new(uint256.Int).Div(new(uint256.Int).Mul(D, big256.BONE), new(uint256.Int).Mul(nCoinsBi, pNew[k]))
			}
			temp, err = geometricMean(xp, true)
			if err != nil {
				return err
			}
			oldVirtualPrice = new(uint256.Int).Div(new(uint256.Int).Mul(big256.BONE, temp), totalSupply)
			if oldVirtualPrice.Cmp(big256.BONE) > 0 && new(uint256.Int).Sub(
				new(uint256.Int).Mul(
					big256.U2, oldVirtualPrice,
				), big256.BONE,
			).Cmp(xcpProfit) > 0 {
				packedPrices = big256.U0
				for k := 0; k < nCoins-1; k += 1 {
...
func getCryptoFee(minFee, maxFee, gamma, xp0, xp1 *uint256.Int) *uint256.Int {
	...
	f = f.Mul(big256.BONE, big256.U4).Mul(f, xp0).Div(f, f1).Mul(f, xp1).Div(f, f1).Sub(
	...
	fee.Sub(big256.BONE, f).Mul(maxFee, fee).Add(fee, new(uint256.Int).Mul(minFee, f)).Div(fee, big256.BONE)
	return fee
}
