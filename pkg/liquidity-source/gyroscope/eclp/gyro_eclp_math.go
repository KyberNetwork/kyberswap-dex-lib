	...
	termNp, err = math.NewSignedFixedPointCalculator(termNp).
		SubNormalWith(
			math.NewSignedFixedPointCalculator(x).
				MulDownMagU(y).
				MulDownMagU(new(int256.Int).Mul(p.C, g._number_2)).
				MulDownMagU(p.S),
		).Result()
	if err != nil {
		return nil, err
...
func (g *gyroECLPMath) calcInvariantSqrt(x, y *int256.Int, p *params, d *derivedParams) (*int256.Int, *int256.Int,
	error) {
	var val *int256.Int
	{
