...
func (g *gyroECLPMath) virtualOffset0(p *ECLParams, d *ECLDerivedParams, r *Vector2) (*int256.Int, error) {
	termXp, err := math.NewSignedFixedPointCalculator(d.TauBeta.X).DivXpU(d.DSq).Result()
	if err != nil {
		return nil, err
	}
...
func (g *gyroECLPMath) virtualOffset1(p *ECLParams, d *ECLDerivedParams, r *Vector2) (*int256.Int, error) {
	termXp, err := math.NewSignedFixedPointCalculator(d.TauAlpha.X).DivXpU(d.DSq).Result()
	if err != nil {
		return nil, err
	}
...
func (g *gyroECLPMath) calcMinAtxAChiySqPlusAtxSq(x, y *int256.Int, p *ECLParams, d *ECLDerivedParams) (*int256.Int,
	error) {
	termNp, err := math.NewSignedFixedPointCalculator(x).
		MulUpMagU(x).
		MulUpMagU(p.C).
...
func (g *gyroECLPMath) calcMinAtyAChixSqPlusAtySq(x, y *int256.Int, p *ECLParams, d *ECLDerivedParams) (*int256.Int,
	error) {
	termNp, err := math.NewSignedFixedPointCalculator(x).
		MulUpMagU(x).
		MulUpMagU(p.S).
...
func (g *gyroECLPMath) calcInvariantSqrt(x, y *int256.Int, p *ECLParams, d *ECLDerivedParams) (*int256.Int, *int256.Int,
	error) {
	var val *int256.Int
	{
