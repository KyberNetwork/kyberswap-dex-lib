...
var _ = pooltrack.RegisterBackupFactoryCE(weighted.DexType, NewPoolTracker)
...
func (t *PoolTracker) GetNewPoolState(
	...
	// call RPC
	rpcResult, err := t.queryRPC(ctx, p.Address, staticExtra.PoolTypeVer, staticExtra.PoolID, staticExtra.Vault)
	if err != nil {
		return p, err
	}
	...
	extra := weighted.Extra{
		SwapFeePercentage:         rpcResult.SwapFeePercentage,
		ProtocolSwapFeePercentage: rpcResult.ProtocolSwapFeePercentage,
		LastInvariant:             rpcResult.LastInvariant,
		TotalSupply:               rpcResult.TotalSupply,
		Paused:                    !isNotPaused(rpcResult.PausedState),
	}
	...
	reserves, err := t.initReserves(p, rpcResult.PoolTokens)
	if err != nil {
		return p, err
	}

	p.BlockNumber = rpcResult.BlockNumber
