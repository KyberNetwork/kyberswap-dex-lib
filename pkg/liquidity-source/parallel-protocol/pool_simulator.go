...
func NewPoolSimulator(p entity.Pool) (*PoolSimulator, error) {
	...
	return &PoolSimulator{
		...
		gas:        extra.Gas,
		Parallelizer: extra.Parallelizer,
	}, nil
}
...
func (s *PoolSimulator) CalcAmountOut(params pool.CalcAmountOutParams) (*pool.CalcAmountOutResult, error) {
	...
	collatStablecoinIssued := s.Parallelizer.Collaterals[collateral].StablecoinsIssued
	otherStablecoinIssued := new(uint256.Int).Sub(s.Parallelizer.TotalStablecoinIssued, collatStablecoinIssued)
	fees := s.Parallelizer.Collaterals[collateral].Fees
	stablecoinCap := s.Parallelizer.Collaterals[collateral].StablecoinCap
...
func (s *PoolSimulator) _getBurnOracle(collateral string) (*uint256.Int, *uint256.Int, error) {
	var oracleValue *uint256.Int
	minRatio := newBASE18()
	for collat := range s.Parallelizer.Collaterals {
...
func (s *PoolSimulator) _readMint(collateral string) (*uint256.Int, error) {
	configOracle := s.Parallelizer.Collaterals[collateral].Config
...
func (p *PoolSimulator) UpdateBalance(params pool.UpdateBalanceParams) {
	if strings.EqualFold(params.TokenAmountIn.Token, p.Info.Tokens[len(p.Info.Tokens)-1]) { // agToken
		p.Parallelizer.Collaterals[params.TokenAmountOut.Token].StablecoinsIssued.Sub(
			p.Parallelizer.Collaterals[params.TokenAmountOut.Token].StablecoinsIssued,
			uint256.MustFromBig(params.TokenAmountIn.Amount),
		)
		p.Parallelizer.TotalStablecoinIssued.Sub(
			p.Parallelizer.TotalStablecoinIssued,
			uint256.MustFromBig(params.TokenAmountIn.Amount),
		)
	} else {
		p.Parallelizer.Collaterals[params.TokenAmountIn.Token].StablecoinsIssued.Add(
			p.Parallelizer.Collaterals[params.TokenAmountIn.Token].StablecoinsIssued,
			uint256.MustFromBig(params.TokenAmountOut.Amount),
		)
		p.Parallelizer.TotalStablecoinIssued.Add(
			p.Parallelizer.TotalStablecoinIssued,
			uint256.MustFromBig(params.TokenAmountOut.Amount),
		)
	}
