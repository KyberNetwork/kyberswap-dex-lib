...
func (t *PoolTracker) getNewPoolState(
	...
	parallelizerState := ParallelizerState{
	...
	for i := range collateralList {
		parallelizerState.Collaterals[hexutil.Encode(collateralList[i][:])] = CollateralState{
			...
			Config: Oracle{
				...
				OracleFeed: t.getOracleFeed(0, i, collateralConfigs[i],  chainlinks, morphos, maxes),
				TargetFeed: t.getOracleFeed(1, i, collateralConfigs[i],  chainlinks, morphos, maxes),
...
func (t *PoolTracker) getOracleFeed(oracleOrTarget int, index int, decodedOracleConfig DecodedOracleConfig, chainlinks [2][]Chainlink, morphos [2][]Morpho, maxes [2][]*uint256.Int) OracleFeed {
