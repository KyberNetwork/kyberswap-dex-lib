...
var (
	...
	poolSim    = lo.Must(NewPoolSimulator(poolEntity, valueobject.ChainIDBase))
)
...
func TestCalcAmountOut(t *testing.T) {
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Equal(t, bignumber.NewBig(tc.expectedAmountOut), result.TokenAmountOut.Amount)
		})
	}
}
