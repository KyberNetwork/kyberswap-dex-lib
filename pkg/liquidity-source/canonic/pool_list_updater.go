...
func (u *PoolsListUpdater) initPools(ctx context.Context, addresses []common.Address) ([]entity.Pool, error) {
	...
	for i, addr := range addresses {
		...
		req.AddCall(&ethrpc.Call{
			...
			Target: target,
			Method: "baseToken",
		}, []any{&infos[i].baseToken}).AddCall(&ethrpc.Call{
			ABI:    poolABI,
			Target: target,
			Method: "quoteToken",
		}, []any{&infos[i].quoteToken})
	}
	...
	for i, addr := range addresses {
		...
		staticExtra := StaticExtra{
			BaseToken:  hexutil.Encode(info.baseToken[:]),
			QuoteToken: hexutil.Encode(info.quoteToken[:]),
		}
		...
		pools = append(pools, entity.Pool{
			...
			Tokens: []*entity.PoolToken{
				{Address: hexutil.Encode(info.baseToken[:]), Swappable: true},
				{Address: hexutil.Encode(info.quoteToken[:]), Swappable: true},
			},
			Extra: "{}",
		})
	}
