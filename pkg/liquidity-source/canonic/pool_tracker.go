...
func (t *PoolTracker) getPoolState(ctx context.Context, p entity.Pool, blockNumber *big.Int) (entity.Pool, error) {
	...
	for i, r := range resp.Result {
		if !r {
			return p, fmt.Errorf("canonic tracker: batch1 call %d failed for %s", i, maobAddr)
		}
	}
	...
	rungCount := uint16(rungCountBI.Uint64())
	if rungCount == 0 {
		rungCount = 1
	}
	...
	depthReq := t.ethrpcClient.NewRequest().SetContext(ctx)
	if blockNumber != nil {
		depthReq.SetBlockNumber(blockNumber)
	} else if resp.BlockNumber != nil {
		depthReq.SetBlockNumber(resp.BlockNumber)
	}

	depthReq.AddCall(&ethrpc.Call{
		ABI:    maobABI,
		Target: maobAddr,
		Method: maobMethodGetDepth,
		Params: []any{rungCount},
	}, []any{&depth})
	resp, err = depthReq.TryAggregate()
	if err != nil {
		return p, err
	}
	if !resp.Result[0] {
		return p, fmt.Errorf("canonic tracker: getDepth failed for %s", maobAddr)
	}
