...
func (s *PoolSimulator) CloneState() pool.IPoolSimulator {
	...
	cloned.midPrecision = new(uint256.Int).Set(s.midPrecision)
	cloned.takerFee = new(uint256.Int).Set(s.takerFee)
	cloned.feeDenom = new(uint256.Int).Set(s.feeDenom)
	cloned.minQuoteTaker = new(uint256.Int).Set(s.minQuoteTaker)
	cloned.rungDenom = new(uint256.Int).Set(s.rungDenom)
	cloned.priceSigfigs = new(uint256.Int).Set(s.priceSigfigs)

	cloned.askRungs = slices.Clone(s.askRungs)
	cloned.askVolumes = make([]*uint256.Int, len(s.askVolumes))
	for i, v := range s.askVolumes {
		cloned.askVolumes[i] = new(uint256.Int).Set(v)
	}

	cloned.bidRungs = slices.Clone(s.bidRungs)
	cloned.bidVolumes = make([]*uint256.Int, len(s.bidVolumes))
	for i, v := range s.bidVolumes {
		cloned.bidVolumes[i] = new(uint256.Int).Set(v)
	}
