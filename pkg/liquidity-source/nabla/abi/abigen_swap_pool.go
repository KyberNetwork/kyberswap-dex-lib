...
//
// Solidity: event ReserveUpdated(uint256 indexed newReserve, uint256 newReserveWithSlippage, uint256 newTotalLiabilities)
func (_NablaSwapPool *NablaSwapPoolFilterer) FilterReserveUpdated(opts *bind.FilterOpts, newReserve []*big.Int) (*NablaSwapPoolReserveUpdatedIterator, error) {

	var newReserveRule []interface{}
	for _, newReserveItem := range newReserve {
		newReserveRule = append(newReserveRule, newReserveItem)
	}

	logs, sub, err := _NablaSwapPool.contract.FilterLogs(opts, "ReserveUpdated", newReserveRule)
	if err != nil {
		return nil, err
	}
...
//
// Solidity: event ReserveUpdated(uint256 indexed newReserve, uint256 newReserveWithSlippage, uint256 newTotalLiabilities)
func (_NablaSwapPool *NablaSwapPoolFilterer) WatchReserveUpdated(opts *bind.WatchOpts, sink chan<- *NablaSwapPoolReserveUpdated, newReserve []*big.Int) (event.Subscription, error) {
	...
	logs, sub, err := _NablaSwapPool.contract.WatchLogs(opts, "ReserveUpdated", newReserveRule)
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
