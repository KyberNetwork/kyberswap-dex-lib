...
func (f *PoolFactory) newPool(p *abis.GSPFactoryNewGSP, blockNumber uint64) (*entity.Pool, error) {
	poolAddress := strings.ToLower(hexutil.Encode(p.GSP[:]))
	staticExtraBytes, err := json.Marshal(shared.StaticExtra{
		PoolId: poolAddress,
		Type:   shared.SubgraphPoolTypeDodoGasSaving,
	})
	if err != nil {
