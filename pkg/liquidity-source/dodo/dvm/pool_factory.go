...
func (f *PoolFactory) newPool(p *abis.DVMFactoryNewDVM, blockNumber uint64) (*entity.Pool, error) {
	poolAddress := strings.ToLower(hexutil.Encode(p.Dvm[:]))
	staticExtraBytes, err := json.Marshal(shared.StaticExtra{
		PoolId: poolAddress,
		Type:   shared.SubgraphPoolTypeDodoVendingMachine,
	})
	if err != nil {
