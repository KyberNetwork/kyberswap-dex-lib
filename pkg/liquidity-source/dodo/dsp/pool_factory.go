...
func (f *PoolFactory) newPool(p *abis.DSPFactoryNewDSP, blockNumber uint64) (*entity.Pool, error) {
	poolAddress := strings.ToLower(hexutil.Encode(p.DSP[:]))
	staticExtraBytes, err := json.Marshal(shared.StaticExtra{
		PoolId: poolAddress,
		Type:   shared.SubgraphPoolTypeDodoStable,
	})
	if err != nil {
