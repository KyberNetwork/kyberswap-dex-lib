...
func (t *PoolTracker) BootstrapPoolState(
	...
	extraBytes, err := json.Marshal(buildRamsesExtra(rpcData, ticks256))
	if err != nil {
		logger.WithFields(logger.Fields{
			"poolAddress": p.Address,
			"error":       err,
...
func buildRamsesExtra(rpcData *FetchRPCResult, ticks []uniswapv3.TickU256) uniswapv3.ExtraTickU256 {
...
func (t *PoolTracker) FetchPoolTicks(ctx context.Context, p entity.Pool) (entity.Pool, error) {
	...
	for _, t := range extra.Ticks {
		ticksToRefetch = append(ticksToRefetch, t.Index)
	}
...
func (t *PoolTracker) updateState(ctx context.Context, p entity.Pool, ticksBasedPool tickspkg.TicksBasedPool) (entity.Pool,
	...
	sort.Slice(ticks256, func(i, j int) bool {
		return ticks256[i].Index < ticks256[j].Index
	})

	extraBytes, err := json.Marshal(buildRamsesExtra(rpcState, ticks256))
	if err != nil {
