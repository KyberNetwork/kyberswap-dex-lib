...
type (
	PoolTracker struct {
		...
		// logDecoder   ILogDecoder // skipping log decoder for now to keep it simpler unless we fetch it
...
func (d *PoolTracker) updatePool(p entity.Pool, reserveData ReserveData, fee uint64,
	...
	p.Timestamp = int64(maxUint32(uint32(p.Timestamp), reserveData.BlockTimestampLast))
...
func (d *PoolTracker) getReservesFromRPCNode(ctx context.Context, poolAddress string) (ReserveData, *big.Int, error) {
	...
	req.AddCall(&ethrpc.Call{
		ABI:    pairABI, // will define in abi.go
		Target: poolAddress,
