...
func TestCalcAmountIn(t *testing.T) {
	t.Run("curve-stable-ng and curve-stable-meta-ng", func(t *testing.T) {
		// init base pool simulator
		basePoolData := `
        {
            "address": "0x383e6b4437b59fff47b619cba855ca29342a8559",
            "amplifiedTvl": 15230456.3638247,
            "exchange": "curve-stable-ng",
            "type": "curve-stable-ng",
            "timestamp": 1752046593,
            "reserves": [
                "6596844237987",
                "8488370101464",
                "15010621768138115163097113"
            ],
            "tokens": [
                {
            ...
            "extra": "{\"InitialA\":\"100000\",\"FutureA\":\"500000\",\"InitialATime\":1743933983,\"FutureATime\":1745126502,\"SwapFee\":\"1000000\",\"AdminFee\":\"5000000000\",\"OffpegFeeMultiplier\":\"50000000000\",\"RateMultipliers\":[\"1000000000000000000000000000000\",\"1000000000000000000000000000000\"]}",
            "staticExtra": "{\"APrecision\":\"100\",\"IsNativeCoins\":[false,false]}",
            "blockNumber": 22880177
        }`
		var basePoolEntity entity.Pool
		err := json.Unmarshal([]byte(basePoolData), &basePoolEntity)
		require.Nil(t, err)
		basePoolSim, err := stableng.NewPoolSimulator(basePoolEntity)
		require.Nil(t, err)
		baseSimsByAddress := map[string]pool.IPoolSimulator{
			basePoolEntity.Address: basePoolSim,
		}

		// init meta pool simulator
		metaPoolData := `
		{
            "address": "0x9e10f9fb6f0d32b350cee2618662243d4f24c64a",
            "amplifiedTvl": 172567.15876308695,
            "exchange": "curve-stable-meta-ng",
            "type": "curve-stable-meta-ng",
            "timestamp": 1752046602,
            "reserves": [
                "84401399278587251786",
                "107280554742478708149",
                "190577437534465844859"
            ],
            "tokens": [
                {
            ...
            "extra": "{\"InitialA\":\"15000\",\"FutureA\":\"15000\",\"InitialATime\":0,\"FutureATime\":0,\"SwapFee\":\"4000000\",\"AdminFee\":\"5000000000\",\"OffpegFeeMultiplier\":\"20000000000\",\"RateMultipliers\":[\"1000000000000000000\",\"1004967714250081535\"]}",
            "staticExtra": "{\"APrecision\":\"100\",\"IsNativeCoins\":[false,false],\"BasePool\":\"0x383e6b4437b59fff47b619cba855ca29342a8559\",\"underlyingTokens\":[\"0x4591dbff62656e7859afe5e45f6f47d3669fbb28\",\"0x6c3ea9036406852006290770bedfcaba0e23a0e8\",\"0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48\"]}",
            "blockNumber": 22880178
        }`
		var poolEntity entity.Pool
		err = json.Unmarshal([]byte(metaPoolData), &poolEntity)
		require.Nil(t, err)
		var e StaticExtra
		err = json.Unmarshal([]byte(poolEntity.StaticExtra), &e)
		require.Nil(t, err)
		...
		baseToken0 := basePoolEntity.Tokens[0].Address // PYUSD
		baseToken1 := basePoolEntity.Tokens[1].Address // USDC
		...
		testCases := []struct {
			...
			// meta pool swap
			// those tests are generated by go to the contract and call `get_dx` function
			{
				name:             "meta to lp",
				tokenIn:          metaToken,
				tokenOut:         lpToken,
				amountOut:        "50000000000000000",
				expectedAmountIn: "50185373836289237",
			},
			{
				name:             "lp to meta",
				tokenIn:          lpToken,
				tokenOut:         metaToken,
				amountOut:        "50000000000000000",
				expectedAmountIn: "49855861202670640",
			},
			{
				name:             "meta to lp large amount",
				tokenIn:          metaToken,
				tokenOut:         lpToken,
				amountOut:        "50000000000000000000",
				expectedAmountIn: "50381287598101126711",
			},
			{
				name:             "lp to meta large amount",
				tokenIn:          lpToken,
				tokenOut:         metaToken,
				amountOut:        "50000000000000000000",
				expectedAmountIn: "50207879669965984362",
			},
			// meta -> base
			// those tests are generated by go to the contract and call `get_dx_underlying` function
			{
				name:             "meta to base 0",
				tokenIn:          metaToken,
				tokenOut:         baseToken0,
				amountOut:        "50000",
				expectedAmountIn: "49940670854869778",
			},
			{
				name:             "meta to base 1",
				tokenIn:          metaToken,
				tokenOut:         baseToken1,
				amountOut:        "50000",
				expectedAmountIn: "49938086088216335",
			},
			// base -> meta
			{
				name:             "base 0 to meta",
			...
			},
			{
				...
				tokenOut:  baseToken0,
				amountOut: "50000",
				wantErr:   ErrTokenToUnderlyingNotSupported,
			},
			{
				name:      "base to lp",
				tokenIn:   baseToken0,
		...
		for _, tc := range testCases {
			t.Run(tc.name, func(t *testing.T) {
				t.Parallel()
				out, err := testutil.MustConcurrentSafe(t, func() (*pool.CalcAmountInResult, error) {
				...
				if tc.wantErr != nil {
					...
					assert.Equal(t, bignumber.NewBig10(tc.expectedAmountIn), out.TokenAmountIn.Amount)
					assert.Equal(t, tc.tokenIn, out.TokenAmountIn.Token)
				}
			})
		}
