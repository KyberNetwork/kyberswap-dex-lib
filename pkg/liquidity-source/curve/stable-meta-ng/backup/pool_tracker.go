...
import (
	...
	stablemetang "github.com/KyberNetwork/kyberswap-dex-lib/pkg/liquidity-source/curve/stable-meta-ng"
...
var _ = pooltrack.RegisterBackupFactoryCE(stablemetang.DexType, NewPoolTracker)
...
func (t *PoolTracker) getNewPoolState(
	...
	var staticExtra stablemetang.StaticExtra
	...
	req := t.ethrpcClient.NewRequest().SetContext(ctx).SetOverrides(overrides).
		...
		AddCall(&ethrpc.Call{
			ABI:    *stablemetang.CurveStableMetaNGABI,
			Target: p.Address,
...
func (t *PoolTracker) updateRateMultipliers(lg logger.Logger, extra *stablemetang.Extra, numTokens int, customRates []*big.Int) error {
	extra.RateMultipliers = make([]uint256.Int, numTokens)
	lg.Debugf("pool use stored rate %v", customRates)

	for i := range numTokens {
		if customRates[i] == nil {
			return stablemetang.ErrInvalidStoredRates
		}
