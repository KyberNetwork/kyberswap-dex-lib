...
var _ = pooltrack.RegisterFactoryCE(twocryptong.DexType, NewPoolTracker)
...
func buildPoolState(lg logger.Logger, p entity.Pool, d *rpcData) (entity.Pool, error) {
	var extra = twocryptong.Extra{
		...
		InitialGamma:       new(uint256.Int).And(number.SetFromBig(d.initialAGamma), twocryptong.PriceMask),
		InitialAGammaTime:  d.initialAGammaTime.Int64(),
		FutureA:            number.SetFromBig(new(big.Int).Rsh(d.futureAGamma, 128)),
		FutureGamma:        new(uint256.Int).And(number.SetFromBig(d.futureAGamma), twocryptong.PriceMask),
		...
		UseCustomMath:      twocryptong.UseCustomMath(d.math),
	}
