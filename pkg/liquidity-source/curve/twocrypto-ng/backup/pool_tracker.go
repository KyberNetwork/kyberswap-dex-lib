...
var _ = pooltrack.RegisterBackupFactoryCE(twocryptong.DexType, NewPoolTracker)
...
func (t *PoolTracker) getNewPoolState(
	...
	calls := t.ethrpcClient.NewRequest().SetContext(ctx).SetOverrides(overrides).SetFrom(shared.AddrDummy).
		AddCall(&ethrpc.Call{
			ABI:    *twocryptong.CurveTwocryptoNGABI,
			Target: p.Address,
			Method: poolMethodD,
	...
	var extra = twocryptong.Extra{
		InitialA:           number.SetFromBig(new(big.Int).Rsh(initialAGamma, 128)),
		InitialGamma:       new(uint256.Int).And(number.SetFromBig(initialAGamma), twocryptong.PriceMask),
		InitialAGammaTime:  initialAGammaTime.Int64(),
		FutureA:            number.SetFromBig(new(big.Int).Rsh(futureAGamma, 128)),
		FutureGamma:        new(uint256.Int).And(number.SetFromBig(futureAGamma), twocryptong.PriceMask),
		...
		UseCustomMath:      twocryptong.UseCustomMath(math),
	}
