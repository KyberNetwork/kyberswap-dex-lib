...
func (t *Curve) getTwoPoolData(ctx context.Context, pool entity.Pool) (interface{}, []*big.Int, error) {
	...
	callParamsFactory := repository.CallParamsFactory(abis.CurveTwo, pool.Address)
	calls = append(calls,
	...
	for i := 0; i < nCoins-1; i++ {
		calls = append(calls,
			callParamsFactory("price_scale", &priceScale[i], nil),
			callParamsFactory("price_oracle", &priceOracle[i], nil),
			callParamsFactory("last_prices", &lastPrices[i], nil),
