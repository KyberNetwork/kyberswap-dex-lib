...
func NewDexPriceAggregatorUniswapV3Reader(scanService *service.ScanService) *DexPriceAggregatorUniswapV3Reader {
	return &DexPriceAggregatorUniswapV3Reader{
		abi:         abis.SynthetixDexPriceAggregatorUniswapV3,
	...
	calls := []*repository.CallParams{
		{
			...
			Params: nil,
			Output: &dexPriceAggregator.DefaultPoolFee,
		},
		{
			ABI:    r.abi,
			Target: address,
			...
			Params: nil,
			Output: &dexPriceAggregator.UniswapV3Factory,
		},
		{
			ABI:    r.abi,
			Target: address,
			...
			Params: nil,
			Output: &dexPriceAggregator.Weth,
		},
	}
		...
		tokenCalls := []*repository.CallParams{
			{
				...
				Output: &overriddenPoolForRoutes[i],
			},
		}
...
func (r *DexPriceAggregatorUniswapV3Reader) readPoolData(
	...
	for i, pool := range poolAddresses {
		tokenCalls := []*repository.TryCallParams{
			{
	...
	if err := r.scanService.TryAggregateForce(ctx, false, calls); err != nil {
		return err
	}
...
func _identifyRouteFromPoolKey(_poolKey PoolKey) string {
	return hex.EncodeToString(crypto.Keccak256(abiutils.EncodePacked(_poolKey.token0.Bytes(), _poolKey.token1.Bytes())))
}
