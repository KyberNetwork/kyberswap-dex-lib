...
func (s *Scanner) newPool(address string, vault *Vault) (*entity.Pool, error) {
	...
	return &entity.Pool{
		...
		Type:      constant.PoolTypes.MadMex,
		Tokens:    poolTokens,
		Reserves:  reserves,
...
func (s *Scanner) updateReserves(ctx context.Context) error {
	...
	if len(pools) == 0 {
		return errors.New("no madmex pool found")
	}
...
func (s *Scanner) getVault(ctx context.Context, address string) (*Vault, error) {
	...
	usdg, err := s.usdgReader.Read(ctx, vault.USDGAddress.String())
	if err != nil {
		return nil, err
	}
...
func (s *Scanner) getVaultPriceFeed(ctx context.Context, address string, tokens []string) (*VaultPriceFeed, error) {
	...
	if !isAddressZero(vaultPriceFeed.ChainlinkFlagsAddress) {
		chainlinkFlags, err := s.chainlinkFlagsReader.Read(ctx, vaultPriceFeed.ChainlinkFlagsAddress.String())
		if err != nil {
			return nil, err
		}
	...
	if !isAddressZero(vaultPriceFeed.BNBBUSDAddress) {
		bnbBusd, err := s.pancakePairReader.Read(ctx, vaultPriceFeed.BNBBUSDAddress.String())
		if err != nil {
			return nil, err
		}
	...
	if !isAddressZero(vaultPriceFeed.BTCBNBAddress) {
		btcBnb, err := s.pancakePairReader.Read(ctx, vaultPriceFeed.BTCBNBAddress.String())
		if err != nil {
			return nil, err
		}
	...
	if !isAddressZero(vaultPriceFeed.ETHBNBAddress) {
		ethBnb, err := s.pancakePairReader.Read(ctx, vaultPriceFeed.ETHBNBAddress.String())
		if err != nil {
			return nil, err
		}
