...
func (f *spfav2Finder) bestMultiPathRouteV1(
	...
	for _, amountInPerSplit := range splits {
		...
		if err == nil && bestAddedPath.CompareTo(bestPath, input.GasInclude && data.PriceAvailable(bestAddedPath.Output.Token)) < 0 {
		...
		if err = bestMultiPathRoute.AddPath(ctx, data.PoolBucket, bestPath, data.SwapLimits); err != nil {
			return nil, err
		}
	}
	return bestMultiPathRoute, nil
...
func (f *spfav2Finder) bestPathExactInV1(
	...
	for _, path := range paths {
		if path != nil && path.CompareTo(bestPath, input.GasInclude && data.PriceAvailable(path.Output.Token)) < 0 {
			bestPath = path
		}
	}
	if bestPath == nil {
