...
import (
	...
	"github.com/KyberNetwork/kyberswap-aggregator/internal/pkg/core/balancerstable"
	"github.com/KyberNetwork/kyberswap-aggregator/internal/pkg/core/balancerweighted"
	"github.com/KyberNetwork/kyberswap-aggregator/internal/pkg/core/camelot"
	...
	curveTwo "github.com/KyberNetwork/kyberswap-aggregator/internal/pkg/core/curve-two"
	"github.com/KyberNetwork/kyberswap-aggregator/internal/pkg/core/dmm"
	"github.com/KyberNetwork/kyberswap-aggregator/internal/pkg/core/dodo"
	"github.com/KyberNetwork/kyberswap-aggregator/internal/pkg/core/fraxswap"
	"github.com/KyberNetwork/kyberswap-aggregator/internal/pkg/core/gmx"
	"github.com/KyberNetwork/kyberswap-aggregator/internal/pkg/core/lido"
	"github.com/KyberNetwork/kyberswap-aggregator/internal/pkg/core/limitorder"
	"github.com/KyberNetwork/kyberswap-aggregator/internal/pkg/core/madmex"
	"github.com/KyberNetwork/kyberswap-aggregator/internal/pkg/core/makerpsm"
	"github.com/KyberNetwork/kyberswap-aggregator/internal/pkg/core/metavault"
	"github.com/KyberNetwork/kyberswap-aggregator/internal/pkg/core/platypus"
	poolPkg "github.com/KyberNetwork/kyberswap-aggregator/internal/pkg/core/pool"
	"github.com/KyberNetwork/kyberswap-aggregator/internal/pkg/core/promm"
	...
	"github.com/KyberNetwork/kyberswap-aggregator/internal/pkg/core/univ3"
	"github.com/KyberNetwork/kyberswap-aggregator/internal/pkg/core/velodrome"
...
// NewPools receives []entity.Pool, initializes []poolPkg.IPool based on pool type
func (f PoolFactory) NewPools(entityPools []entity.Pool) []poolPkg.IPool {
	curveBasePoolByAddress := f.getCurveBasePoolByAddress(entityPools)
	curvePlainOraclePoolByAddress := f.getCurvePlainOraclePoolByAddress(entityPools)

	basePools := combinePoolsMap(curveBasePoolByAddress, curvePlainOraclePoolByAddress)

	corePools := make([]poolPkg.IPool, 0, len(entityPools))
	for _, entityPool := range entityPools {
		switch entityPool.Type {
		case constant.PoolTypes.CurveBase:
			corePool, ok := curveBasePoolByAddress[entityPool.Address]
			if !ok {
				continue // NOTE: already warned in getCurveBasePoolByAddress
			}

			corePools = append(corePools, corePool)

		case constant.PoolTypes.CurvePlainOracle:
			corePool, ok := curvePlainOraclePoolByAddress[entityPool.Address]
			if !ok {
		...
		case constant.PoolTypes.CurveMeta:
			corePool, err := f.newCurveMeta(entityPool, basePools)
			if err != nil {
		...
		default:
			corePool, err := f.newPool(entityPool)
			if err != nil {
				logger.Debugf(err.Error())
				continue
			}
...
func (f PoolFactory) NewPoolByAddress(entityPools []entity.Pool) map[string]poolPkg.IPool {
	pools := f.NewPools(entityPools)
