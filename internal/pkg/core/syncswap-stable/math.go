...
func getExactQuote(
	...
	amountOut = new(big.Int).Div(new(big.Int).Sub(new(big.Int).Sub(adjustedReserveOut, y), constant.One), tokenOutPrecisionMultiplier)
	...
	for i := 0; i < 256; i++ {
		...
		var dp = new(big.Int).Div(
			new(big.Int).Div(
				new(big.Int).Mul(
					new(big.Int).Div(
						new(big.Int).Mul(d, d), xp0), d), xp1), constant.Four)
		prevD = d
		...
		d = new(big.Int).Div(
			new(big.Int).Mul(
				new(big.Int).Add(
					...
					new(big.Int).Mul(constant.Two, dp),
				), d),
			new(big.Int).Add(
				new(big.Int).Mul(twoThousandMinusOne, d),
				new(big.Int).Mul(constant.Three, dp),
			),
		)
...
func getY(x, d *big.Int) *big.Int {
	...
	var c = new(big.Int).Div(
		new(big.Int).Mul(d, d),
		new(big.Int).Mul(x, constant.Two),
	)
	//c = (c * d) / 4000;
	...
	for i := 0; i < 256; i++ {
		...
		y = new(big.Int).Div(
			...
			new(big.Int).Sub(new(big.Int).Add(new(big.Int).Mul(y, constant.Two), b), d),
		)
...
func within1(a, b *big.Int) bool {
	if a.Cmp(b) > 0 {
		return new(big.Int).Sub(a, b).Cmp(constant.One) <= 0
	}

	return new(big.Int).Sub(b, a).Cmp(constant.One) < 0
}
