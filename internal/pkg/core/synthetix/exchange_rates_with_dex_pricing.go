...
func (er *ExchangeRatesWithDexPricing) _formatAggregatorAnswer(currencyKey string, rate *big.Int) (*big.Int, error) {
	...
	if decimals == 0 || decimals == 18 {
		...
		multiplier := constant.TenPowInt(18 - decimals)
		result = new(big.Int).Mul(result, multiplier)
	} else if decimals > 18 {
		// decrease precision to 18
		divisor := constant.TenPowInt(decimals - 18)
...
func (er *ExchangeRatesWithDexPricing) _effectiveValueAndRates(
	...
	if sourceCurrencyKey == destinationCurrencyKey {
		...
		if destinationRate.Cmp(constant.Zero) > 0 {
...
func (er *ExchangeRatesWithDexPricing) _dexPriceDestinationValue(
	...
	sourceAmountInEquivalent := new(big.Int).Div(new(big.Int).Mul(sourceAmount, constant.TenPowInt(sourceEquivalent.Decimals)), unit())

	twapWindow := er.getAtomicTwapWindow()
	if twapWindow.Cmp(constant.Zero) == 0 {
	...
	if twapValueInEquivalent.Cmp(constant.Zero) == 0 {
		return nil, ErrDexPriceZero
	}

	// Similar to source amount, normalize decimals back to internal unit for output amount
	return new(big.Int).Div(new(big.Int).Mul(twapValueInEquivalent, unit()), constant.TenPowInt(destEquivalent.Decimals)), nil
}
