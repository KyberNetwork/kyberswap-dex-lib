...
func (c *Pool) GetLpToken() string {
	return c.GetAddress()

}

func (c *Pool) GetMidPrice(tokenIn string, tokenOut string, base *big.Int) *big.Int {
	var tokenInIndex = c.GetTokenIndex(tokenIn)
	var tokenOutIndex = c.GetTokenIndex(tokenOut)
	var reserveIn = c.Info.Reserves[tokenInIndex]
	var reserveOut = c.Info.Reserves[tokenOutIndex]
	var ret = new(big.Int).Mul(base, reserveOut)
	ret = new(big.Int).Div(ret, reserveIn)
	return ret
}

func (c *Pool) CalcExactQuote(tokenIn string, tokenOut string, base *big.Int) *big.Int {
	var tokenInIndex = c.GetTokenIndex(tokenIn)
	var tokenOutIndex = c.GetTokenIndex(tokenOut)
	var reserveIn = c.Info.Reserves[tokenInIndex]
	var reserveOut = c.Info.Reserves[tokenOutIndex]
...
func NewPoolSimulator(entityPool entity.Pool) (*Pool, error) {
	...
	swapFeeFl := new(big.Float).Mul(big.NewFloat(entityPool.SwapFee), constant.BoneFloat)
...
func (c *Pool) CalcAmountOut(tokenAmountIn pool.TokenAmount, tokenOut string) (*pool.CalcAmountOutResult, error) {
	...
	fmt.Printf("DEBUG: tokenIn %s tokenOut %s amountIn %s ..... amountOut %s\n\n", tokenAmountIn.Token, tokenOut, tokenAmountIn.Amount.String(), amountOut)
...
func (c *Pool) GetMetaInfo(_ string, _ string) interface{} {
	return Meta{
...
func calcBptOutGivenExactTokensIn(amp *big.Int, balances []*big.Int, amountsIn []*big.Int, bptTotalSupply, invariant, swapFeePercentage *big.Int) (*big.Int, *big.Int) {
	...
	for i, balance := range balances {
		...
		if balanceRatiosWithFee[i].Cmp(invariantRatioWithFees) > 0 {
			nonTaxableAmount := MulDownFixed(balance, new(big.Int).Sub(invariantRatioWithFees, constant.BONE))
			taxableAmount := new(big.Int).Sub(amountsIn[i], nonTaxableAmount)
			amountInWithoutFee = new(big.Int).Add(
				nonTaxableAmount,
				MulDownFixed(
					taxableAmount,
					new(big.Int).Sub(constant.BONE, swapFeePercentage),
				),
			)
		} else {
	...
	if invariantRatio.Cmp(constant.BONE) > 0 {
		return MulDownFixed(bptTotalSupply, new(big.Int).Sub(invariantRatio, constant.BONE)), feeAmountIn
	} else {
...
func (c *Pool) _getProtocolPoolOwnershipPercentage(balances []*big.Int) (*big.Int, *big.Int) {
	...
	if swapFeeGrowthInvariantDelta.Cmp(constant.Zero) < 0 {
	...
	if nonExemptYieldGrowthInvariantDelta.Cmp(constant.Zero) < 0 {
...
func (c *Pool) _adjustedBalance(balance *big.Int, cache *TokenRateCache) *big.Int {
