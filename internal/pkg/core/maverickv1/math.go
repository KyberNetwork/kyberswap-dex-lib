...
func GetAmountOut(
	...
	for delta.Excess.Cmp(zeroBI) > 0 {
		...
		if isForPricing && counter > MaxSwapIterationCalculation {
			return zeroBI, zeroBI, ErrMaxSwapIteration
		}
	}
...
func swapTick(delta *Delta, state *MaverickPoolState) (*Delta, error) {
	...
	if delta.DecrementTick {
		activeTick = new(big.Int).Sub(state.ActiveTick, constant.One)
	}
	...
	delta.SqrtUpperTickPrice, err = tickPrice(state.TickSpacing, new(big.Int).Add(activeTick, constant.One))
	if err != nil {
		return nil, err
	}
	...
	if newDelta.Excess.Cmp(zeroBI) != 0 {
		if newDelta.TokenAIn {
			state.ActiveTick = new(big.Int).Add(state.ActiveTick, constant.One)
		}
...
func computeSwapExactOut(
	...
	feeBasis, err := mulDiv(
		binAmountIn,
		state.Fee,
		new(big.Int).Sub(constant.TenPowInt(18), state.Fee),
		true,
	)
	if err != nil {
		return nil, err
...
func computeSwapExactIn(
	...
	tmp, err := mul(amountIn, new(big.Int).Sub(constant.TenPowInt(18), state.Fee))
	if err != nil {
		return nil, err
	}
	if tmp.Cmp(binAmountIn) >= 0 {
		feeBasis, err = mulDiv(binAmountIn, state.Fee, new(big.Int).Sub(constant.TenPowInt(18), state.Fee), true)
		if err != nil {
			return nil, err
		}
		delta.DeltaInErc = new(big.Int).Add(binAmountIn, feeBasis)
		...
		binAmountIn, err = mul(amountIn, new(big.Int).Sub(constant.TenPowInt(18), state.Fee))
		if err != nil {
			return nil, err
		}
...
func amountToBin(deltaInErc, feeBases *big.Int, state *MaverickPoolState) (*big.Int, error) {
	...
	if protocolFeeRatio.Cmp(zeroBI) != 0 {
		tmpMul, err := mul(feeBases, new(big.Int).Mul(protocolFeeRatio, constant.TenPowInt(15)))
		if err != nil {
			return nil, err
		}
		return clip(deltaInErc, new(big.Int).Add(tmpMul, constant.One)), nil
	}
...
func currentTickLiquidity(activeTick *big.Int, state *MaverickPoolState) (*big.Int, *big.Int, *big.Int, *big.Int, []Bin, error) {
	...
	sqrtUpperTickPrice, err = tickPrice(state.TickSpacing, new(big.Int).Add(activeTick, constant.One))
	if err != nil {
		return nil, nil, nil, nil, nil, err
	}
...
func tickPrice(tickSpacing *big.Int, activeTick *big.Int) (*big.Int, error) {
	...
	var ratio *big.Int
	if new(big.Int).And(tick, utils.NewBig("0x1")).Cmp(zeroBI) != 0 {
		ratio = utils.NewBig("0xfffcb933bd6fad9d3af5f0b9f25db4d6")
	} else {
		ratio = utils.NewBig("0x100000000000000000000000000000000")
	}

	if new(big.Int).And(tick, utils.NewBig("0x2")).Cmp(zeroBI) != 0 {
		ratio.Mul(ratio, utils.NewBig("0xfff97272373d41fd789c8cb37ffcaa1c"))
		ratio.Rsh(ratio, 128)
	}
	if new(big.Int).And(tick, utils.NewBig("0x4")).Cmp(zeroBI) != 0 {
		ratio.Mul(ratio, utils.NewBig("0xfff2e50f5f656ac9229c67059486f389"))
		ratio.Rsh(ratio, 128)
	}
	if new(big.Int).And(tick, utils.NewBig("0x8")).Cmp(zeroBI) != 0 {
		ratio.Mul(ratio, utils.NewBig("0xffe5caca7e10e81259b3cddc7a064941"))
		ratio.Rsh(ratio, 128)
	}
	if new(big.Int).And(tick, utils.NewBig("0x10")).Cmp(zeroBI) != 0 {
		ratio.Mul(ratio, utils.NewBig("0xffcb9843d60f67b19e8887e0bd251eb7"))
		ratio.Rsh(ratio, 128)
	}
	if new(big.Int).And(tick, utils.NewBig("0x20")).Cmp(zeroBI) != 0 {
		ratio.Mul(ratio, utils.NewBig("0xff973b41fa98cd2e57b660be99eb2c4a"))
		ratio.Rsh(ratio, 128)
	}
	if new(big.Int).And(tick, utils.NewBig("0x40")).Cmp(zeroBI) != 0 {
		ratio.Mul(ratio, utils.NewBig("0xff2ea16466c9838804e327cb417cafcb"))
		ratio.Rsh(ratio, 128)
	}
	if new(big.Int).And(tick, utils.NewBig("0x80")).Cmp(zeroBI) != 0 {
		ratio.Mul(ratio, utils.NewBig("0xfe5dee046a99d51e2cc356c2f617dbe0"))
		ratio.Rsh(ratio, 128)
	}
	if new(big.Int).And(tick, utils.NewBig("0x100")).Cmp(zeroBI) != 0 {
		ratio.Mul(ratio, utils.NewBig("0xfcbe86c7900aecf64236ab31f1f9dcb5"))
		ratio.Rsh(ratio, 128)
	}
	if new(big.Int).And(tick, utils.NewBig("0x200")).Cmp(zeroBI) != 0 {
		ratio.Mul(ratio, utils.NewBig("0xf987a7253ac4d9194200696907cf2e37"))
		ratio.Rsh(ratio, 128)
	}
	if new(big.Int).And(tick, utils.NewBig("0x400")).Cmp(zeroBI) != 0 {
		ratio.Mul(ratio, utils.NewBig("0xf3392b0822b88206f8abe8a3b44dd9be"))
		ratio.Rsh(ratio, 128)
	}
	if new(big.Int).And(tick, utils.NewBig("0x800")).Cmp(zeroBI) != 0 {
		ratio.Mul(ratio, utils.NewBig("0xe7159475a2c578ef4f1d17b2b235d480"))
		ratio.Rsh(ratio, 128)
	}
	if new(big.Int).And(tick, utils.NewBig("0x1000")).Cmp(zeroBI) != 0 {
		ratio.Mul(ratio, utils.NewBig("0xd097f3bdfd254ee83bdd3f248e7e785e"))
		ratio.Rsh(ratio, 128)
	}
	if new(big.Int).And(tick, utils.NewBig("0x2000")).Cmp(zeroBI) != 0 {
		ratio.Mul(ratio, utils.NewBig("0xa9f746462d8f7dd10e744d913d033333"))
		ratio.Rsh(ratio, 128)
	}
	if new(big.Int).And(tick, utils.NewBig("0x4000")).Cmp(zeroBI) != 0 {
		ratio.Mul(ratio, utils.NewBig("0x70d869a156ddd32a39e257bc3f50aa9b"))
		ratio.Rsh(ratio, 128)
	}
	if new(big.Int).And(tick, utils.NewBig("0x8000")).Cmp(zeroBI) != 0 {
		ratio.Mul(ratio, utils.NewBig("0x31be135f97da6e09a19dc367e3b6da40"))
		ratio.Rsh(ratio, 128)
	}
	if new(big.Int).And(tick, utils.NewBig("0x10000")).Cmp(zeroBI) != 0 {
		ratio.Mul(ratio, utils.NewBig("0x9aa508b5b7e5a9780b0cc4e25d61a56"))
		ratio.Rsh(ratio, 128)
	}
	if new(big.Int).And(tick, utils.NewBig("0x20000")).Cmp(zeroBI) != 0 {
		ratio.Mul(ratio, utils.NewBig("0x5d6af8dedbcb3a6ccb7ce618d14225"))
		ratio.Rsh(ratio, 128)
	}
	if new(big.Int).And(tick, utils.NewBig("0x40000")).Cmp(zeroBI) != 0 {
		ratio.Mul(ratio, utils.NewBig("0x2216e584f630389b2052b8db590e"))
		ratio.Rsh(ratio, 128)
	}
	if new(big.Int).And(tick, utils.NewBig("0x80000")).Cmp(zeroBI) != 0 {
		ratio.Mul(ratio, utils.NewBig("0x48a1703920644d4030024fe"))
		ratio.Rsh(ratio, 128)
	}
	if new(big.Int).And(tick, utils.NewBig("0x100000")).Cmp(zeroBI) != 0 {
		ratio.Mul(ratio, utils.NewBig("0x149b34ee7b4532"))
		ratio.Rsh(ratio, 128)
	}
...
func nextActive(binMap map[string]*big.Int, tick *big.Int, isRight bool) *big.Int {
	...
	if isRight {
		refTick = new(big.Int).Add(tick, constant.One)
	}
	...
	if nextWord != nil && nextWord.Cmp(zeroBI) != 0 {
		...
		if posFirst.Cmp(zeroBI) < 0 {
			pos = new(big.Int).Add(pos, constant.One)
		}
		nextTick = new(big.Int).Div(pos, Kinds)
		if posFirst.Cmp(zeroBI) < 0 {
			nextTick = new(big.Int).Sub(nextTick, constant.One)
		}
	}
...
func lsb(x *big.Int) *big.Int {
	...
	if tmpX.And(tmpX, utils.NewBig("0xffffffffffffffffffffffffffffffff")).Cmp(zeroBI) > 0 {
		r.Sub(r, big.NewInt(128))
	} else {
		tmpX.Rsh(tmpX, 128)
	}
	if tmpX.And(tmpX, utils.NewBig("0xffffffffffffffff")).Cmp(zeroBI) > 0 {
		r.Sub(r, big.NewInt(64))
	} else {
		tmpX.Rsh(tmpX, 64)
	}
	if tmpX.And(tmpX, utils.NewBig("0xffffffff")).Cmp(zeroBI) > 0 {
		r.Sub(r, big.NewInt(32))
	} else {
		tmpX.Rsh(tmpX, 32)
	}
	if tmpX.And(tmpX, utils.NewBig("0xffff")).Cmp(zeroBI) > 0 {
		r.Sub(r, big.NewInt(16))
	} else {
		tmpX.Rsh(tmpX, 16)
	}
	if tmpX.And(tmpX, utils.NewBig("0xff")).Cmp(zeroBI) > 0 {
		r.Sub(r, big.NewInt(8))
	} else {
		tmpX.Rsh(tmpX, 8)
	}
	if tmpX.And(tmpX, utils.NewBig("0xf")).Cmp(zeroBI) > 0 {
		r.Sub(r, big.NewInt(4))
	} else {
		tmpX.Rsh(tmpX, 4)
	}
	if tmpX.And(tmpX, utils.NewBig("0x3")).Cmp(zeroBI) > 0 {
		r.Sub(r, big.NewInt(2))
	} else {
		tmpX.Rsh(tmpX, 2)
	}
	if tmpX.And(tmpX, utils.NewBig("0x1")).Cmp(zeroBI) > 0 {
...
func msb(x *big.Int) *big.Int {
	...
	if tmpX.Cmp(utils.NewBig("0x100000000000000000000000000000000")) >= 0 {
		tmpX.Rsh(tmpX, 128)
		r.Add(r, big.NewInt(128))
	}
	if tmpX.Cmp(utils.NewBig("0x10000000000000000")) >= 0 {
		tmpX.Rsh(tmpX, 64)
		r.Add(r, big.NewInt(64))
	}
	if tmpX.Cmp(utils.NewBig("0x100000000")) >= 0 {
		tmpX.Rsh(tmpX, 32)
		r.Add(r, big.NewInt(32))
	}
	if tmpX.Cmp(utils.NewBig("0x10000")) >= 0 {
		tmpX.Rsh(tmpX, 16)
		r.Add(r, big.NewInt(16))
	}
	if tmpX.Cmp(utils.NewBig("0x100")) >= 0 {
		tmpX.Rsh(tmpX, 8)
		r.Add(r, big.NewInt(8))
	}
	if tmpX.Cmp(utils.NewBig("0x10")) >= 0 {
		tmpX.Rsh(tmpX, 4)
		r.Add(r, big.NewInt(4))
	}
	if tmpX.Cmp(utils.NewBig("0x4")) >= 0 {
		tmpX.Rsh(tmpX, 2)
		r.Add(r, big.NewInt(2))
	}
	if tmpX.Cmp(utils.NewBig("0x2")) >= 0 {
	...
	if a.Cmp(zeroBI) == 0 || new(big.Int).Div(product, a).Cmp(b) == 0 {
		if product.Cmp(zeroBI) == 0 {
			...
			if ceil && new(big.Int).Mod(product, c).Cmp(zeroBI) != 0 {
				return new(big.Int).Add(new(big.Int).Div(product, c), constant.One), nil
			} else {
...
func sqrt(x *big.Int) *big.Int {
	...
	result := big.NewInt(1)
	if xAux.Cmp(utils.NewBig("0x100000000000000000000000000000000")) >= 0 {
		xAux.Rsh(xAux, 128)
		result.Lsh(result, 64)
	}
	if xAux.Cmp(utils.NewBig("0x10000000000000000")) >= 0 {
		xAux.Rsh(xAux, 64)
		result.Lsh(result, 32)
	}
	if xAux.Cmp(utils.NewBig("0x100000000")) >= 0 {
		xAux.Rsh(xAux, 32)
		result.Lsh(result, 16)
	}
	if xAux.Cmp(utils.NewBig("0x10000")) >= 0 {
		xAux.Rsh(xAux, 16)
		result.Lsh(result, 8)
	}
	if xAux.Cmp(utils.NewBig("0x100")) >= 0 {
		xAux.Rsh(xAux, 8)
		result.Lsh(result, 4)
	}
	if xAux.Cmp(utils.NewBig("0x10")) >= 0 {
		xAux.Rsh(xAux, 4)
		result.Lsh(result, 2)
	}
	if xAux.Cmp(utils.NewBig("0x8")) >= 0 {
		result.Lsh(result, 1)
	}
...
func mul(a, b *big.Int) (*big.Int, error) {
	if new(big.Int).Mod(
		...
		Unit,
	).Cmp(utils.NewBig("499999999999999999")) > 0 {
...
func mulUpFixed(a, b *big.Int) (*big.Int, error) {
	...
	if a.Cmp(zeroBI) == 0 || new(big.Int).Div(product, a).Cmp(b) == 0 {
		...
		if product.Cmp(zeroBI) == 0 {
			...
			result := new(big.Int).Sub(abs(product), constant.One)
	...
	if a.Cmp(zeroBI) == 0 || new(big.Int).Div(product, a).Cmp(b) == 0 {
		if product.Cmp(zeroBI) == 0 {
			...
			return new(big.Int).Add(
				new(big.Int).Div(new(big.Int).Sub(product, constant.One), One),
				constant.One,
			), nil
		}
	}
...
func scaleFromAmount(amount *big.Int, decimals uint8) (*big.Int, error) {
	...
	if decimals > 18 {
		scalingFactor = new(big.Int).Mul(
			constant.TenPowInt(18),
			constant.TenPowInt(decimals-18),
		)
		return sDivDownFixed(amount, scalingFactor)
	} else {
		scalingFactor = new(big.Int).Mul(
			constant.TenPowInt(18),
			constant.TenPowInt(18-decimals),
		)
