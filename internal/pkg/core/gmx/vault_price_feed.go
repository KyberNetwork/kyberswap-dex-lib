...
// VaultPriceFeed
// https://github.com/gmx-io/gmx-contracts/blob/master/contracts/core/VaultPriceFeed.sol
type VaultPriceFeed struct {
	BNB                        string
	BTC                        string
	ETH                        string
	FavorPrimaryPrice          bool
	IsAmmEnabled               bool
	IsSecondaryPriceEnabled    bool
	MaxStrictPriceDeviation    *big.Int
	PriceSampleSpace           *big.Int
	SpreadThresholdBasisPoints *big.Int
	UseV2Pricing               bool
	...
	ChainlinkFlags *ChainlinkFlags

	SecondaryPriceFeed        IFastPriceFeed
	SecondaryPriceFeedVersion int
...
func (pf *VaultPriceFeed) GetPrice(token string, maximise bool, includeAmmPrice bool, _ bool) (*big.Int, error) {
	...
	if adjustmentBps.Cmp(constant.Zero) > 0 {
...
func (pf *VaultPriceFeed) getPriceV1(token string, maximise bool, includeAmmPrice bool) (*big.Int, error) {
	...
	if includeAmmPrice && pf.IsAmmEnabled {
		...
		if ammPrice.Cmp(constant.Zero) > 0 {
...
func (pf *VaultPriceFeed) getPrimaryPrice(token string, maximise bool) (*big.Int, error) {
	...
	for i := big.NewInt(0); i.Cmp(pf.PriceSampleSpace) < 0; i = new(big.Int).Add(i, big.NewInt(1)) {
		...
		var p *big.Int
		if i.Cmp(constant.Zero) == 0 {
			p = priceFeed.LatestAnswer()

			if p.Cmp(constant.Zero) <= 0 {
...
func (pf *VaultPriceFeed) getPairPrice(pair *PancakePair, divByReserve0 bool) *big.Int {
	...
	if divByReserve0 {
		if reserve0.Cmp(constant.Zero) == 0 {
			return constant.Zero
		}
	...
	if reserve1.Cmp(constant.Zero) == 0 {
		return constant.Zero
	}
